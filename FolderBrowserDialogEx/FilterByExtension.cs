...
namespace Ionic.Utils
{
	...
	public class FilterByExtension : IFolderFilter
	{
			...
			IntPtr pidlItem)		// The item's PIDL.
		{
			...
			{
				if (sDisplay.ToUpper().EndsWith("." + ValidExtensions[i].ToUpper()))
					return 0;
			}
			...
			out UInt32 pgrfFlags)	// One or more SHCONTF values that specify which classes of objects to enumerate.
		{
            ...
            ShellLib.ShellApi.STRRET ptrDisplayName;
            isf.GetDisplayNameOf(pidlFolder, (uint)ShellLib.ShellApi.SHGNO.SHGDN_NORMAL | (uint)ShellLib.ShellApi.SHGNO.SHGDN_FORPARSING, out ptrDisplayName);

            String sDisplay;
            ShellLib.ShellApi.StrRetToBSTR(ref ptrDisplayName, (IntPtr)0, out sDisplay);

            if (sDisplay != null && DontExpandZip && sDisplay.ToUpper().EndsWith(".ZIP"))
                pgrfFlags = 0;  // show nothing
            else
